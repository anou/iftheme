...
<div id="content">
	...
	?>
	<div class="author">
		<h1><?php _e('About:','iftheme'); ?> <?php echo $curauth->display_name; ?></h1>
	...
	<div id="recent-author-posts">
		<h3><?php _e('Recent Posts by ','iftheme'); echo $curauth->display_name; ?></h3>
		<?php if ( have_posts() ) : while ( have_posts() ) : the_post(); /* Displays the most recent posts by that author. Note that this does not display custom content types */ ?>
			...
			if ($count == "5") // Number of posts to display
				...
				<div class="post-meta">
					<p><?php _e('Written on ','iftheme'); the_time('F j, Y'); _e(' at ','iftheme'); the_time() ?></p>
					<p><?php _e('Categories: ','iftheme'); the_category(', ');?></p>
					<p><?php the_tags('<br />Tags: ', ', ', ' '); ?></p>
				</div><!--.postMeta-->
			<?php $count++; } ?>
		<?php endwhile; else: ?>
			<p><?php _e('No posts by ','iftheme'); echo $curauth->display_name; ?> yet.</p>
		<?php endif; ?>
	</div><!--#recentPosts-->

	<div id="recent-author-comments">
		<h3><?php _e('Recent Comments by ','iftheme'); echo $curauth->display_name; ?></h3>
		<?php
		...
		<ul>
			<?php
				...
				echo  '<li class="recentcomments">' . sprintf(__('%1$s on %2$s','iftheme'), get_comment_date(), '<a href="'. get_comment_link($comment->comment_ID) . '">' . get_the_title($comment->comment_post_ID) . '</a>') . '</li>';
			endforeach; else: ?>
             	<p><?php _e('No comments by ','iftheme'); echo $curauth->display_name; ?></p>
			<?php endif; ?>
		</ul>
