...
----------------------------
	ORIGINAL Author URI: http://konstruktors.com/blog/
...
class widget_context {
	...
	function save_widget_context() {
		...
		foreach($new_widget_context_settings as $widget_id => $widget_context) {
			...
			//added by anou
			//If not admin always show
			if($current_user->ID != 1)
				$widget_context['incexc'] = 'selected';
	...
	function check_widget_visibility($vis_settings = array()) {
		...
		$do_show_by_antenna = false;
		$do_show_on_front = false;
		...
		if(function_exists('multi_antennas')){
			if (!empty($vis_settings['country']) && $multi) {
				if (is_front_page() && $paged < 2) $do_show_on_front = true;
				if (is_home() && $paged < 2) $do_show_on_front = true;
			}
		}
		

		// Combine all context checks
		if ($do_show_by_word_count || $do_show_by_url || $do_show_by_select || $do_show_on_front || $do_show_by_antenna)
			$one_is_true = true;
		elseif (!$do_show_by_word_count || !$do_show_by_url || !$do_show_by_select || !$do_show_on_front || !$do_show_by_antenna)
			$one_is_true = false;
...
//$this->printAdminOptions();
		...
		if(!isset($antennas['aid'])) { //multi-antennas
			foreach($antennas as $a => $vals) {
				$out .= $this->make_simple_checkbox_antenna($options, $wid, $groupA, $vals['aid'], get_cat_name( $vals['aid'] ));
			}
		} else {//simple antenna
			$out .= $this->make_simple_checkbox_antenna($options, $wid, $groupA, $antennas['aid'], get_cat_name( $antennas['aid'] ));
		}
		$out .= '</p></div>';
...
*/
	...
	//ANTENNAS
	function get_antennas_list() {
		$antennas = '';
		if(function_exists('get_antennas_details')){
			$antennas = get_antennas_details();
		}
		return $antennas;
	}
